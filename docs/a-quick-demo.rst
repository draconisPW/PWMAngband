...
"floor". There will also be a few ``:`` symbols which represent rubble.
Rubble comes in two varieties, "passable rubble" and just "rubble",
distinguished by color. Both block vision, but you can move through
passable rubble.
...
cursor always starts on the square containing your character. Pressing space
will cycle through the interesting locations nearby. Press ``q`` or ESCAPE
to exit the look command.
...
with look command, ``l``; see the targeting section of the manual
for more details on that). Use the ``R`` key, then return, to
...
The best resource for learning these things is the manual that includes this
demo. The in-game help, which you access by pressing ``?``, does have a list of
the commands with the key and very brief description for each.