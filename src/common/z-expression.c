...
static expression_operator_t expression_operator_from_token(const char *token)
{
    ...
    switch (token[0])
    {
        case '+': result = OPERATOR_ADD; break;
        case '-': result = OPERATOR_SUB; break;
        case '*': result = OPERATOR_MUL; break;
        case '/': result = OPERATOR_DIV; break;
        case 'n':
        case 'N': result = OPERATOR_NEG; break;
