...
struct sdlpui_control_funcs {
     ...
     * shouldn't be passed on to another handler.  Otherwise, return SDL_FALSE.
     ...
     */
    /*
     * Respond to a key event.  Return SDL_TRUE if the event was handled and
     * should not be passed on to another handler.  Otherwise, return SDL_FALSE.
     * Can be NULL if the control does not do anything with a key event
     * and wants the dialog or window to handle the event.
     */
    SDL_bool (*handle_key)(struct sdlpui_control *c, struct sdlpui_dialog *d,
     ...
     * return SDL_FALSE.  Can be NULL if the control does not do anything with
     * a text input event and wants the dialog or window to handle the
     * event.
     */
    SDL_bool (*handle_textin)(struct sdlpui_control *c, struct sdlpui_dialog *d,
     ...
     *     b) If first is not SDL_FALSE, returns the one-based index of the first
     ...
     * focus.
     */
    int (*get_interactable_component)(struct sdlpui_control *c, SDL_bool first);
    /*
     * Step (forward if forward is not SDL_FALSE; backward otherwise; never wrap
     ...
     * focus to the result of the step.  Return SDL_TRUE if stepping was
     * possible or SDL_FALSE otherwise.  May be NULL:  callers will then assume
     ...
     */
    SDL_bool (*set_disabled)(struct sdlpui_control *c, struct sdlpui_dialog *d,
...
struct sdlpui_menu_button {
    ...
    SDL_bool disabled;    /* if not SDL_FALSE, no response to events and different look */
...
void sdlpui_create_menu_ranged_int(struct sdlpui_control *c,
        ...
        void (*callback)(struct sdlpui_control*, struct sdlpui_dialog*,
        struct sdlpui_window*), int tag, SDL_bool disabled, int curr_value,
