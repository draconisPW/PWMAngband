 ...
 * May be NULL.
 * \return SDL_TRUE if other is a descendant of ancestor; return SDL_FALSE if
 ...
 * \param give_key_focus causes, if not SDL_FALSE, the dialog to be given key focus
...
/**
 * Remove the dialog, any of its child dialogs, and, if all_parents is not SDL_FALSE,
 ...
 * \return SDL_TRUE if the event is handled and does not need further processing by
 * the window; otherwise return SDL_FALSE.
 */
 ...
 * controls.
 * \param vertical will, if not SDL_FALSE, causes the controls in the new menu to
 * be layed out in a single column; if SDL_FALSE, it causes the controls to layed
 * out in a single row.
 * \param border will, if not SDL_FALSE, cause a border to be drawn about the menu.
