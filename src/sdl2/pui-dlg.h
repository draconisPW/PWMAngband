...
struct sdlpui_dialog_funcs {
     ...
     */
    SDL_bool (*handle_key)(struct sdlpui_dialog *d, struct sdlpui_window *w,
     ...
     * window to handle it.
     */
    SDL_bool (*handle_textin)(struct sdlpui_dialog *d, struct sdlpui_window *w,
    ...
    void (*goto_first_control)(struct sdlpui_dialog *d,
        struct sdlpui_window *w);
    /*
     * If forward is not SDL_FALSE, go to the dialog's next (with wrap around)
     * control after c that can accept focus.  If forward is SDL_FALSE, go to
