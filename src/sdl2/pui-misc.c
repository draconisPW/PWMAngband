    ...
    if (!my_codes.lock) {
        ...
        if (!my_codes.lock) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                "Could not create code mutex in sdlpui_init(): %s", SDL_GetError());
        }
    ...
    if (lock) {
        if (!SDL_LockMutex(lock)) {
            ...
            if (SDL_UnlockMutex(lock)) {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                    "Could not release code mutex in sdlpui_quit()");
            }
            for (i = 0; i < n; ++i) {
            ...
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                "Could not acquire code mutex in sdlpui_quit()");
        }
    }
}
        ...
        if (ilo == ihi) {
            ...
            if (my_codes.count == my_codes.alloc) {
                ...
                if (my_codes.alloc == 0) {
                    ...
                    my_codes.alloc < (size_t)-1 / sizeof(*new_entries))
                {
                    new_alloc = (my_codes.alloc < SDL_MAX_UINT32 / 2)?
                        my_codes.alloc + my_codes.alloc : SDL_MAX_UINT32;
                } else {
                    /*
                     * Will exceed the range of an Uint32 or size_t.
                     * Give up.
                     */
                    SDL_free(name_copy);
                    break;
            ...
            if (ilo < my_codes.count) {
                SDL_memmove(my_codes.entries + ilo + 1, my_codes.entries + ilo,
                    (my_codes.count - ilo) * sizeof(*my_codes.entries));
            }
            code = ++my_codes.count;
