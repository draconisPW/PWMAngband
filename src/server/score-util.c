 ...
 *
 * a is a pointer to a high score record cast to a const void*.
 * b is a pointer to a high score record cast to a const void*.
 ...
 *
 * s points to the score record to check.
 * Return true if the record is valid. Otherwise, return false.
 */
static bool highscore_valid(const struct high_score *s)
{
    if (s->what[0])
    {
        ...
        if (!memchr(s->what, '\0', sizeof(s->what)) || !memchr(s->pts, '\0', sizeof(s->pts)) ||
            !memchr(s->gold, '\0', sizeof(s->gold)) || !memchr(s->turns, '\0', sizeof(s->turns)) ||
            !memchr(s->day, '\0', sizeof(s->day)) || !memchr(s->who, '\0', sizeof(s->who)) ||
            !memchr(s->uid, '\0', sizeof(s->uid)) || !memchr(s->sex, '\0', sizeof(s->sex)) ||
            !memchr(s->p_r, '\0', sizeof(s->p_r)) || !memchr(s->p_c, '\0', sizeof(s->p_c)) ||
            !memchr(s->cur_lev, '\0', sizeof(s->cur_lev)) || !memchr(s->cur_dun, '\0', sizeof(s->cur_dun)) ||
            !memchr(s->max_lev, '\0', sizeof(s->max_lev)) || !memchr(s->max_dun, '\0', sizeof(s->max_dun)) ||
         ...
         * short enough that they will not reach most restrictive
         * bounds for a long with c99.
         */
        strtol(s->pts, &p_end, 0);
        if (*p_end || !s->pts[0]) return false;
        strtol(s->gold, &p_end, 0);
        if (*p_end || !s->gold[0]) return false;
        strtol(s->turns, &p_end, 0);
        if (*p_end || !s->turns[0]) return false;
        strtol(s->uid, &p_end, 0);
        if (*p_end || !s->uid[0]) return false;
        strtol(s->p_r, &p_end, 0);
        if (*p_end || !s->p_r[0]) return false;
        strtol(s->p_c, &p_end, 0);
        if (*p_end || !s->p_c[0]) return false;
        strtol(s->cur_lev, &p_end, 0);
        if (*p_end || !s->cur_lev[0]) return false;
        strtol(s->cur_dun, &p_end, 0);
        if (*p_end || !s->cur_dun[0]) return false;
        strtol(s->max_lev, &p_end, 0);
        if (*p_end || !s->max_lev[0]) return false;
        strtol(s->max_dun, &p_end, 0);
        if (*p_end || !s->max_dun[0]) return false;
    }
    else
    {
        /* It is an empty record. All the fields must be empty strings. */
        if (s->pts[0] || s->gold[0] || s->turns[0] || s->day[0] || s->who[0] || s->uid[0] ||
            s->sex[0] || s->p_r[0] || s->p_c[0] || s->cur_lev[0] || s->cur_dun[0] ||
            s->max_lev[0] || s->max_dun[0] || s->how[0])
        {
            return false;
        }
    }
 ...
 *
 * scores is the array of high scores.
 * sz is the number of high scores in scores. It may be zero.
...
bool highscore_regularize(struct high_score scores[], size_t sz)
{
    ...
    /* Inclusive bounds for the indices that were copied out of and not subsequently overwritten */
    ...
    for (i_in = 0, i_out = 0; i_in < sz; ++i_in)
    {
        if (!highscore_valid(scores + i_in))
        {
            ...
            memset(scores + i_in, 0, sizeof(scores[i_in]));
            irregular = true;
            continue;
        }

        /* Skip empty score records. */
        if (!scores[i_in].what[0]) continue;

        if (last_not_empty && highscore_cmp(last_not_empty, scores + i_in) > 0)
            out_of_order = true;
        if (i_out != i_in)
        {
            /* Copy the record to cover gaps from invalid or empty records. */
            scores[i_out] = scores[i_in];
            if (first_copied > i_in) first_copied = i_in;
            else if (first_copied == i_out) ++first_copied;
            last_copied = i_in;
            irregular = true;
        }
    ...
    if (out_of_order)
    {
        /* Can only be out of order if there are two or more records that are not empty. */
        my_assert(i_out >= 2 && i_out <= sz);
    ...
    if (first_copied <= last_copied)
    {
         ...
         */
        my_assert(last_copied < sz);
        memset(scores + first_copied, 0, (last_copied - first_copied + 1) * sizeof(scores[i_out]));
    }
 ...
 *
 * entry points to the new score which is valid and not empty.
 * scores is an array of sz valid score records, sorted from best to
 * worst with any empty records at the end.
 * sz is the number of score records scores holds. sz must be positive.
...
size_t highscore_where(const struct high_score *entry, const struct high_score scores[], size_t sz)
{
    ...
    for (i = 0; i < sz; i++)
    {
        ...
        /* *entry is not empty so it would replace the first empty record. */
        if (!scores[i].what[0]) return i;

        /* A winning record appears before any that did not win. */
        score_winner = strcmp(scores[i].how, WINNING_HOW) == 0;
        if (entry_winner != score_winner)
        {
            if (entry_winner) return i;
            continue;
        }

        /*
         * A record with more points appears before any with
