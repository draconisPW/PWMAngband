...
struct visuals_color_cycle
{
    ...
    size_t max_steps;       /* The size of the steps array. */
    char *cycle_name;       /* The identifier of the color cycle. */
    byte invalid_color;     /* A value to mark locations in steps that should not be considered a valid color. */
};
 ...
 *
 * name The identifier for the new color cycle.
 * step_count The maximum number of steps in the cycle.
 * invalid_color A value used to mark steps that should not be considered
...
static struct visuals_color_cycle *visuals_color_cycle_new(const char *name,
    const size_t step_count, const byte invalid_color)
{
 ...
 *
 * cycle The color cycle to deallocate.
 */
 ...
 *
 * original The color cycle to copy.
 * return: A new color cycle instance, containing the same cycle information as
 *         original. NULL is returned on error.
 */
static struct visuals_color_cycle *visuals_color_cycle_copy(const struct visuals_color_cycle *original)
{
	...
	copy = visuals_color_cycle_new(original->cycle_name, valid_colors, original->invalid_color);
 ...
 *
 * cycle The color cycle to select a color from.
 * frame An arbitrary value used to select the color step.
 * return: A color or BASIC_COLORS if an error occurred.
 */
static byte visuals_color_cycle_attr_for_frame(const struct visuals_color_cycle *cycle,
    const size_t frame)
{
    size_t step = 0;
...
struct visuals_cycle_group
{
	...
	size_t max_cycles;                      /* The size of the cycles array. */
 ...
 * cycle_count The maximum number of color cycles that will be in the group.
 * return: A new color cycle group or NULL if an error occurred.
 */
static struct visuals_cycle_group *visuals_cycle_group_new(const char *name, const size_t cycle_count)
{
	struct visuals_cycle_group *group = NULL;

	if (cycle_count == 0) return NULL;
...
struct visuals_cycler
{
	...
	size_t max_groups;                      /* Size of the groups array. */
};
 ...
 *
 * group_count The maximum number of groups that the table will contain.
 * return: A new color cycling table or NULL if an error occurred.
 */
static struct visuals_cycler *visuals_cycler_new(const size_t group_count)
{
 ...
 *
 * cycler The cycler to deallocate.
 */
 ...
 *
 * cycler The cycler to search.
 * group_name The name of the group to search.
 * cycle_name The name of the color cycle to find.
 * return: A color cycle or NULL if an error occurred.
 */
static struct visuals_color_cycle *visuals_cycler_cycle_by_name(const struct visuals_cycler *cycler,
 ...
 * table.
 *
 * group_name The color cycle group to search.
 * cycle_name The color cycle to use.
 * frame An arbitrary value used to select which color in the flicker
 *       cycle to return.
 * return: An attribute in the color cycle, or BASIC_COLORS if an error
 *         occurred.
 */
byte visuals_cycler_get_attr_for_frame(const char *group_name, const char *cycle_name,
    const size_t frame)
{
 ...
 * reallocate the visuals_color_cycles_by_race table as needed, using its
 * allocation increment. This table is initially created in the module init
 * function.
 *
 * race The monster race to set the color cycle for.
 * group_name The group of the preferred color cycle.
 * cycle_name The name of the preferred color cycle.
 */
void visuals_cycler_set_cycle_for_race(const struct monster_race *race, const char *group_name,
 ...
 *
 * race The race to get an attribute for.
 * frame An arbitrary value used to select which color in the flicker cycle to return.
 * return: An attribute in the color cycle, or BASIC_COLORS if an error occurred.
 */
byte visuals_cycler_get_attr_for_race(const struct monster_race *race, const size_t frame)
{
	struct visuals_color_cycle *cycle = NULL;
...
{
    byte *cycles;               /* Array of max_cycles * colors_per_cycle colors. */
 ...
 *
 * max_cycles The maximum number of cycles to allow for this table.
 * colors_per_cycle The maximum number of color steps in each cycle.
 * return: A new flicker color table instance, or NULL if one could not be created.
 */
static struct visuals_flicker *visuals_flicker_new(const size_t max_cycles,
    const size_t colors_per_cycle)
{
    struct visuals_flicker *table = NULL;
    size_t cycles_size = 0;

    /* Make sure we can at least support the basic color table. */
    if ((max_cycles < BASIC_COLORS) || (colors_per_cycle == 0)) return NULL;
 ...
 *
 * table The color table to deallocate.
 */
 ...
 *
 * table The color table to update.
 * cycle_index The index of the cycle to update.
 * color_index The step in the given cycle to update.
 * attr The actual color to use.
 */
static void visuals_flicker_set_color(struct visuals_flicker *table, const size_t cycle_index,
    const size_t color_index, const byte attr)
{
    if (table == NULL) return;
    if (cycle_index >= table->max_cycles) return;
    if (color_index >= table->colors_per_cycle) return;
 ...
 *
 * table The color table to use.
 * cycle_index The index of the desired cycle.
 * color_index The desired step in the cycle.
 * return: The color at the given step in the given cycle.
 */
static byte visuals_flicker_get_color(struct visuals_flicker *table, const size_t cycle_index,
    const size_t color_index)
{
    if (table == NULL) return 0;
    if (cycle_index >= table->max_cycles) return 0;
    if (color_index >= table->colors_per_cycle) return 0;
 ...
 * color table.
 *
 * selection_attr The attribute used to select which flicker cycle to use.
 * frame An arbitrary value used to select which color in the flicker cycle to return.
 * return: An attribute in the flicker cycle, or BASIC_COLORS if an error occurred.
 */
byte visuals_flicker_get_attr_for_frame(const byte selection_attr, const size_t frame)
{
    size_t color_index = 0;

    if (visuals_flicker_table == NULL) return BASIC_COLORS;
    if ((size_t)selection_attr >= visuals_flicker_table->max_cycles) return BASIC_COLORS;

    color_index = (frame % visuals_flicker_table->colors_per_cycle);
    return visuals_flicker_get_color(visuals_flicker_table, selection_attr, color_index);
}
 ...
 *
 * context The parser context to free.
 */
 ...
 *
 * context The current visuals parser context.
 * return: A new cycler instance or NULL if an error occurred.
 */
static struct visuals_cycler *visuals_parse_context_convert(const struct visuals_parse_context *context)
{
	...
	for (group = 0; group < group_count; group++)
    {
		...
		const size_t cycle_offset = group * VISUALS_CYCLES_MAX;
		...
		if (cycle_group == NULL) return NULL;
		...
		for (cycle = 0; cycle < cycle_count; cycle++)
        {
			...
			if (cycle_group->cycles[cycle] == NULL) return NULL;
		}
...
static enum parser_error visuals_parse_flicker(struct parser *parser)
{
    ...
    if (context == NULL) return PARSE_ERROR_INTERNAL;

    color_code = parser_getsym(parser, "color");
    if ((color_code == NULL) || (strlen(color_code) == 0)) return PARSE_ERROR_INVALID_COLOR;
...
static enum parser_error visuals_parse_flicker_color(struct parser *parser)
{
    ...
    visuals_flicker_set_color(visuals_flicker_table, context->flicker_cycle_index,
        context->flicker_color_index, (byte)attr);
...
static enum parser_error visuals_parse_cycle(struct parser *parser)
{
	...
	/* Create a new cycle with the maximum number of steps (if we can), otherwise bail. */
	if (context->cycles[context->cycles_index] == NULL)
    {
		context->cycles[context->cycles_index] = visuals_color_cycle_new(parsed_name,
            VISUALS_STEPS_MAX, VISUALS_INVALID_COLOR);

		if (context->cycles[context->cycles_index] == NULL)
			return PARSE_ERROR_INTERNAL;
	}

	/* Find the group that matches the parsed cycle's group or create a new one if not found. */
	...
	for (i = 0; i < VISUALS_CYCLES_MAX; i++)
    {
		...
		if (offset >= context->max_group_cycles) continue;
...
static struct parser *visuals_file_parser_init(void)
{
    ...
    if ((context == NULL) || (parser == NULL)) return NULL;
