...
BOOL SaveWindow_PNG(HWND hWnd, LPSTR lpFileName)
{
    ...
    if (!c) return FALSE;
    c += 1;
    if ((strncmp(c, "png", 3) != 0) && (strncmp(c, "PNG", 3) != 0)) return FALSE;
    ...
    /* We are done with the file pointer, so release all the the PNG Structures */
    if (info_ptr)
    {
